    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" method="POST" action="{{ route('category_links.update',$category->id) }}"  >
                    @csrf
                    @method('PUT')

                    <div class="form-group">
                        <label for="category_link">Category Link</label>
                        <input type="text" class="form-control" id="category_link" autocomplete="off" placeholder="Category Link" name="category_link" value="{{ $category->category_link }}"> 
                    </div>
                    <div class="form-group">
                        <label for="website_id">Website</label>
                        <select class="form-control" name="website_id">
                            @foreach ($websites as $link)
                                <option value="{{ $link->id }}" {{$category->website_id == $link->id  ? 'selected' : ''}}>{{ $link->website_name}}</option>     
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
